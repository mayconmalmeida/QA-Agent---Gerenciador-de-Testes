...
public class SmartPage {
    ...
    public void navegarParaMenu(String menuTexto, String submenuTexto) {
        ...
        try {
            ...
            page.waitForTimeout(1000); // Extra wait for JS rendering
            
            // Debug: tira screenshot para ver onde estamos
            String screenshotPath = "output/screenshots/debug_menu_search.png";
            page.screenshot(new Page.ScreenshotOptions().setPath(java.nio.file.Paths.get(screenshotPath)));
            System.out.println("[SmartPage] Screenshot salvo: " + screenshotPath);
            
            // Tenta encontrar menu com busca flexível
            Locator menu = encontrarElementoPorTexto(menuTexto);
            if (menu == null) {
                // Tenta buscar por parte do texto
                String[] partes = menuTexto.split(" ");
                for (String parte : partes) {
                    if (parte.length() > 3) {
                        menu = encontrarElementoPorTexto(parte);
                        if (menu != null) {
                            System.out.println("[SmartPage] Menu encontrado por parte: " + parte);
                            break;
                        }
                    }
                }
            ...
            if (submenuTexto != null && !submenuTexto.isEmpty()) {
                ...
                if (submenu == null) {
                    // Tenta buscar por parte do texto
                    String[] partes = submenuTexto.split(" ");
                    for (String parte : partes) {
                        if (parte.length() > 3) {
                            submenu = encontrarElementoPorTexto(parte);
                            if (submenu != null) {
                                System.out.println("[SmartPage] Submenu encontrado por parte: " + parte);
                                break;
                            }
                        }
                    }
    ...
    public void navigate(String url) {
        System.out.println("[SmartPage] Navigating to: " + url);
        page.navigate(url);
        page.waitForLoadState(LoadState.NETWORKIDLE);
    }

    /**
     * Clica em elemento com auto-healing
     */
    public void clickWithFallback(String target, br.com.qasuite.domain.StepResult result) throws Exception {
        Exception lastError = null;

        // Tenta clique exato
        try {
            Locator element = encontrarElementoPorTexto(target);
            if (element != null) {
        ...
        // Auto-healing: variações
        List<String> variations = generateVariations(target);
        for (String variation : variations) {
            try {
                Locator element = encontrarElementoPorTexto(variation);
                if (element != null) {
                    element.scrollIntoViewIfNeeded();
                    element.click();
                    result.setElementUsed(variation);
                    System.out.println("[SmartPage] Auto-healing click: '" + variation + "'");
                    return;
                }
            } catch (Exception e) {
                // Continue
        ...
        throw lastError != null ? lastError : new RuntimeException("Element not found: '" + target + "'");
    }

    /**
     * Preenche campo com auto-healing
     */
    public void fillWithFallback(String target, String value, br.com.qasuite.domain.StepResult result) throws Exception {
        Exception lastError = null;

        // Tenta preenchimento exato
        try {
            preencherCampo(target, value);
            result.setElementUsed(target);
            return;
        } catch (Exception e) {
            lastError = e;
        ...
        // Auto-healing: variações do nome do campo
        List<String> variations = generateVariations(target);
        for (String variation : variations) {
            try {
                preencherCampo(variation, value);
                result.setElementUsed(variation);
                System.out.println("[SmartPage] Auto-healing fill: '" + variation + "'");
                return;
            } catch (Exception e) {
                // Continue
            }
        ...
        throw lastError != null ? lastError : new RuntimeException("Field not found: '" + target + "'");
    }

    /**
     * Seleciona opção com auto-healing
     */
    public void selectWithFallback(String target, String value, br.com.qasuite.domain.StepResult result) throws Exception {
        Exception lastError = null;

        try {
            selecionarOpcao(target, value);
            result.setElementUsed(target);
            return;
        } catch (Exception e) {
            lastError = e;
        ...
        for (String variation : variations) {
            try {
                selecionarOpcao(variation, value);
                result.setElementUsed(variation);
                System.out.println("[SmartPage] Auto-healing select: '" + variation + "'");
                return;
            } catch (Exception e) {
                // Continue
            }
        ...
        throw lastError != null ? lastError : new RuntimeException("Select not found: '" + target + "'");
    }

    /**
     * Marca/desmarca checkbox com auto-healing
     */
    public void checkboxWithFallback(String target, boolean check, br.com.qasuite.domain.StepResult result) throws Exception {
        Exception lastError = null;

        try {
            marcarCheckbox(target, check);
            result.setElementUsed(target);
            return;
        } catch (Exception e) {
            lastError = e;
        ...
        for (String variation : variations) {
            try {
                marcarCheckbox(variation, check);
                result.setElementUsed(variation);
                System.out.println("[SmartPage] Auto-healing checkbox: '" + variation + "'");
                return;
            } catch (Exception e) {
                // Continue
            }
        ...
        throw lastError != null ? lastError : new RuntimeException("Checkbox not found: '" + target + "'");
    }

    /**
     * Passa mouse sobre elemento
     */
    public void hover(String target) throws Exception {
        Locator element = encontrarElementoPorTexto(target);
        if (element == null) {
            throw new RuntimeException("Element not found for hover: '" + target + "'");
        }
        element.hover();
    }
    ...
    public String takeScreenshot(String name) {
        String path = "output/screenshots/" + name + "_" + System.currentTimeMillis() + ".png";
     ...
     */
    private java.util.List<String> generateVariations(String original) {
